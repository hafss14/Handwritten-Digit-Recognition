...
def predict_digit():
    ...
    if model_name is None or model_name not in ['knn', 'svm', 'rf']:
...
class Predict():
    ...
    def __call__(self, img, model_name='cnn'):
        ...
        if model_name != 'cnn':
            ...
            classifier = load('./' + str(model_name))

            # print(tensor.shape[])
            # print(classifier.predict([tensor]))
            # print(classifier)

            preds = classifier.predict_proba([tensor])[0]
